...
export function BrandProfileWorkspace({ profile }: { profile: BrandIntelligenceProfile }) {
  ...
  const evidenceCounts=evidenceSummary(profile);
  ...
  return <main className="space-y-7">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center gap-3"><BookOpenCheck className="text-teal-700"/><div><p className="text-xs font-black uppercase tracking-wider text-teal-700">Evidence ledger</p><h2 className="text-2xl font-black">Evidence & Confidence</h2></div></div><p className="mt-3 text-sm text-slate-600">Evidence entries identify origin and review state. A source label does not imply external verification.</p><div className="mt-4 flex flex-wrap gap-4 text-sm">{Object.entries(evidenceCounts).map(([label,count])=><p key={label}><strong>{count}</strong> {label}</p>)}</div><p className="mt-2 text-xs text-slate-500">Sourced facts may still be unverified. Inferred and sourced counts can overlap.</p><details className="mt-4"><summary className="cursor-pointer py-3 font-bold text-teal-700">Review detailed evidence and provenance</summary><div className="mt-5 grid gap-3 md:grid-cols-2">{profile.evidence.map((item) => <EvidenceCard key={item.id} evidence={item}/>)}</div></details></section>
  </main>;
}
