...
export function CandidateRelationshipOverview({ candidate }: { candidate: Candidate360State }) {
  return (
    <section className="grid gap-6 xl:grid-cols-[1.35fr_0.85fr]">
      ...
      {!candidate.hasIntelligence && <div id="assessment-invitation" className="scroll-mt-24 xl:col-span-2"><AssessmentInvitationAction candidateId={candidate.id} existingUrl={candidate.assessmentUrl} session={candidate.assessmentSession} /></div>}
      {candidate.brandStrategy && <div className="xl:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-blue-600">Brand Presentation</p><h2 className="mt-1 text-xl font-black">{candidate.brandStrategy.statusLabel}</h2><p className="mt-2 text-sm text-slate-600">{candidate.brandStrategy.recommendations} AI recommendations · {candidate.brandStrategy.presented} presented · {candidate.brandStrategy.strongInterest} strong-interest · {candidate.brandStrategy.referralSelections} selected for referral</p></div><Link href={candidate.brandStrategy.actionHref} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-black text-white">{candidate.brandStrategy.actionLabel}</Link></div></div>}
