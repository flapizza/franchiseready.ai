...
export class CandidateEngagementPlaybookService {
  ...
  async build(candidateId: string): Promise<CandidateEngagementPlaybook | null> {
    ...
    const drafts = this.compose({ candidateId, stage: candidate.pipelineStage, summary: candidate.intelligence?.executiveSummary ?? "", latest, clicks, meetings: ownMeetings, strategy, referrals });
  ...
  private compose(context: { candidateId: string; stage: string; summary: string; latest?: ReturnType<EmailCommunicationRuntime["load"]>[number]; clicks: number; meetings: Awaited<ReturnType<DemoCalendarRepository["getEvents"]>>; strategy: Awaited<ReturnType<CandidateBrandStrategyRuntime["load"]>>; referrals: ReturnType<CandidateReferralService["getByCandidate"]> }): Draft[] {
    const steps: Draft[] = [];
    ...
    const concern = /family|financial|lifestyle|alignment/i.test(context.summary);
