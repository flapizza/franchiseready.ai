...
export function CommunicationsWorkspacePage({ state }: { state: CommunicationsWorkspaceState }) {
  const [composing, setComposing] = useState(false);
  const [replying, setReplying] = useState(false);
  const [idempotencyKey, setIdempotencyKey] = useState("");
  const [candidateId, setCandidateId] = useState(state.selected?.candidateId ?? state.candidates[0]?.id ?? "");
  ...
  return <main className="space-y-5 p-5 lg:p-7" data-communications-workspace>
    ...
    {composing && <div role="dialog" aria-modal="true" aria-label={replying ? "Draft candidate follow-up" : "Compose email"} className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-4"><div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl"><form action={sendAction} className="space-y-4 p-6"><input type="hidden" name="idempotencyKey" value={idempotencyKey} /><div className="flex items-start justify-between"><div><p className="text-xs font-black uppercase tracking-[.2em] text-blue-600">{replying ? "Message-level follow-up" : "New communication"}</p><h3 className="mt-1 text-2xl font-black">{replying ? "Draft follow-up" : "Compose Email"}</h3></div><button type="button" aria-label="Close composer" onClick={() => setComposing(false)} className="rounded-lg p-2 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-200"><X size={19} /></button></div>
