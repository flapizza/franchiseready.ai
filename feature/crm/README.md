...
Brand Presentation keeps three inputs distinct. **Brand Truth** comes only from the canonical `BrandProfile`: overview, category, business/operating model, customer, staffing, revenue, investment, differentiators, and approved discovery questions. Missing profile facts render as `Not Yet Available` or `Not Yet Profiled`. **Candidate Intelligence** contributes assessment, Discovery, and financial evidence without changing the original AI Match. **Presentation Strategy** deterministically connects those sources into emphasized talking points, concerns, and fit-testing questions.

`CandidateBrandPresentationBrief` is the presentation read model. `CandidateBrandStrategyRuntime.loadPresentation()` builds it outside React and preserves AI rank, score, evidence source labels, consultant order, existing reaction, existing consultant note, and existing shortlist disposition. `/crm/candidates/[candidateId]/strategy/presentation` walks only the Strategy Builder Presentation Set; outside-brand work remains in Referral Studio. Explicit advancement sets `presentedAt`, while presentation start/completion timestamps live on the persistence-neutral `StrategyBuilderRecord`. Adding a brand to the set does not mark it presented.

The analytics chain is AI Recommended → Consultant Selected → Presented → Candidate Reaction → Shortlisted → Referred → Brand Outcome → Awarded. Reaction remains separate evidence and does not retrain or recalculate deterministic demo scoring. Candidate 360, the Strategy Queue, and Mission Control derive progress from this record rather than introducing another `PipelineStage`.

Future `BrandProfile` evolution should add a reusable Brand Presentation Profile sourced from franchisor onboarding interviews, approved brand materials, FDD intelligence, and consultant/franchisor-approved facts. That profile can later reference brochures, franchise websites, videos, FDD summaries, legally appropriate earnings disclosures, territory maps, and validation contacts. Asset management and ingestion remain outside this pack. The deterministic brief builder is also a replaceable boundary for future AI-generated talking points, adaptive questions, objection summaries, next-brand guidance, and follow-up drafting; JSX remains only the rendering layer.

## Governed Brand Intelligence

`BrandIntelligenceProfile` is the governed, candidate-independent projection of canonical `BrandProfile` truth. Each meaningful fact carries source, presentation approval, and verification metadata. `BrandIntelligenceRuntime` is the compatibility boundary during migration: existing scoring continues to consume the same canonical `BrandProfile` financial and operating values, while Presentation Mode and the read-only Brand Library consume approval-filtered governed facts. This prevents duplicate financial truth and makes unsupported fields explicitly unavailable. Profile completeness describes data availability only and never affects AI Match or brand quality.

Governance rules are strict: candidate AI, reactions, match scores, and referral outcomes cannot overwrite Brand Truth; missing data remains missing; internal or needs-review facts are not presented as approved claims; candidate views consume but do not own brand facts. Historical workflows should eventually snapshot the governed profile version ID used for each presentation/referral.

Future FDD ingestion may populate fees, royalties, investment, territory obligations, training, renewal, litigation, restrictions, and properly governed financial-performance disclosures with `fdd` provenance. Future franchisor onboarding is submit → normalize → consultant/admin review → approve for presentation → version. Material storage will later manage brochures, videos, FDDs, earnings materials, validation sheets, territory maps, and decks. Static territory structure is not live availability; the future qualification boundary is governed profile + desired candidate territory + live territory provider.
