...
function CandidateCard({ candidate, dragging, onDragStart, onDragEnd, suppressNavigation }: { candidate: CandidateCRMItem; dragging: boolean; onDragStart: (event: React.DragEvent<HTMLElement>) => void; onDragEnd: () => void; suppressNavigation: boolean }) {
  ...
  return (
    <article draggable data-candidate-id={candidate.id} onDragStart={onDragStart} onDragEnd={onDragEnd} onClickCapture={(event) => { if (suppressNavigation) { event.preventDefault(); event.stopPropagation(); } }} onClick={(event) => { if (!(event.target as HTMLElement).closest("a,button")) window.location.assign(candidate.href); }} className={`group relative cursor-grab rounded-2xl border bg-white p-4 shadow-sm transition duration-200 active:cursor-grabbing hover:-translate-y-0.5 hover:shadow-lg ${dragging ? "scale-[0.98] opacity-50 ring-2 ring-blue-400" : ""} ${candidate.referralReady ? "border-emerald-300 ring-1 ring-emerald-100" : "border-slate-200 hover:border-blue-300"}`}>
      ...
      {candidate.openTaskCount > 0 && <Link href={`/crm/tasks?candidate=${candidate.id}`} className="relative z-10 mt-3 inline-flex rounded-lg bg-blue-50 px-2.5 py-1.5 text-[11px] font-black text-blue-700">{candidate.openTaskCount} open task{candidate.openTaskCount === 1 ? "" : "s"}</Link>}
      <div className="mt-4"><div className="flex items-center justify-between text-[11px]"><span className="font-bold uppercase tracking-wide text-slate-400">{candidate.readinessKind === "assessment" ? "Assessment status" : "Readiness"}</span><span className="font-black text-slate-700">{candidate.readinessLabel}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${candidate.referralReady ? "bg-emerald-500" : "bg-teal-500"}`} style={{ width: `${candidate.readiness ?? 0}%` }} /></div></div>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1050px] border-collapse text-left text-sm">
              <thead className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500"><tr>{["Candidate", "Pipeline Stage", state.readinessHeading ?? "Readiness", "Best Brand", "Last Activity", "Next Action", "Priority", ""].map((label) => <th key={label || "open"} className="border-b border-slate-200 px-4 py-3 font-bold">{label}</th>)}</tr></thead>
              <tbody className="divide-y divide-slate-100">
                {filtered.map((candidate) => (
                  <tr key={candidate.id} data-candidate-id={candidate.id} className="group hover:bg-blue-50/40">
                    <td className="px-4 py-3"><Link href={candidate.href} className="flex items-center gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-900 text-xs font-bold text-white">{candidate.initials}</span><span><span className="block font-bold text-slate-900 group-hover:text-blue-700">{candidate.fullName}</span><span className="block text-xs text-slate-500">{candidate.email} · {candidate.location}</span></span></Link>{candidate.readinessKind === "assessment" && <Link href={candidate.actionHref} className="mt-2 inline-flex min-h-11 items-center rounded-lg bg-blue-50 px-3 text-sm font-bold text-blue-700">{candidate.actionLabel}</Link>}</td>
