...
import { assetIdSchema, brandingSchema, imageReferences, type BrandingSnapshot, type MediaAsset } from './model';
...
export async function resolveMedia(document:EmailDocument,branding?:BrandingSnapshot|null):Promise<Record<string,MediaAsset>> {
  return resolveMediaAssets(imageReferences(document,branding));
}
export async function resolveMediaAssets(assetIds:readonly string[]):Promise<Record<string,MediaAsset>> {
  const ids=[...new Set(assetIds.map(id=>assetIdSchema.parse(id)))];if(!ids.length)return {};
  const {session,db}=await mediaWorkspace();
