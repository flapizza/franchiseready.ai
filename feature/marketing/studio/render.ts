...
const contextSchema = z.object({
  ...
  preview: z.boolean(),
  assets: z.record(z.string(),z.object({deliveryUrl:z.string().url()}).passthrough()).optional(),
}).strict();
    ...
    switch (node.type) {
      ...
      case "emailImage":
        if (node.attrs.assetId) {
          const asset=ctx.assets?.[node.attrs.assetId];
          if(!asset)throw new Error('Image is unavailable in this workspace.');
          const url=new URL(asset.deliveryUrl);
          if(!((url.protocol==='https:' || (url.protocol==='http:' && ['127.0.0.1','localhost'].includes(url.hostname))) && /^\/storage\/v1\/object\/public\/studio-delivery\/[a-f0-9-]{36}\/asset_[a-f0-9]{32}\/1\/email\.png$/.test(url.pathname)))throw new Error('Invalid delivery image URL.');
          const img=`<img src="${escape(asset.deliveryUrl)}" alt="${escape(node.attrs.alt)}" width="${node.attrs.width}" style="display:block;width:100%;max-width:${node.attrs.width}px;height:auto;border:0;margin:${node.attrs.alignment==='center'?'0 auto':node.attrs.alignment==='right'?'0 0 0 auto':'0'}">`;
          return node.attrs.href?link(node.attrs.href,img):img;
        }
        // An unconnected placeholder cannot enter a future delivery.
        if (!ctx.preview) throw new Error("Media resolution is not enabled.");
        return `<table role="presentation" align="${node.attrs.alignment}" width="${node.attrs.width}" style="width:100%;max-width:${node.attrs.width}px;margin-bottom:16px"><tr><td style="padding:24px;background:#F1F5F9;color:#475569;text-align:center">${escape(node.attrs.alt || "Image placeholder")}<br><small>Choose an image from the media library</small></td></tr></table>`;
