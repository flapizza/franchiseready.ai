...
export function renderResponse(outcome: UnsubscribeOutcome, test = false): string {
  const success = outcome === "success";
  const heading = success ? (test ? "Test email acknowledged." : "You have been unsubscribed.") : "This link is unavailable.";
  const message = success
    ? (test ? "This test link does not change marketing subscription preferences." : "You will no longer receive marketing emails from this sender.")
